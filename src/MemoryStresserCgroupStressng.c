/*
* This program demonstrates how to create and manage a cgroup for memory control in Linux and run multiple stress-ng
 * processes within that cgroup to simulate memory pressure. It begins by setting up a custom cgroup directory at
 * `/sys/fs/cgroup/memory_stress` to manage processes that consume memory. The program then enables the memory
 * controller for the cgroup, ensuring that memory limits can be applied. Next, it sets a memory limit of 1GB for
 * the cgroup, ensuring that the total memory usage of processes within this cgroup does not exceed this limit.
 * The program runs two separate `stress-ng` processes concurrently, each attempting to allocate 1GB of memory,
 * to simulate a high-memory usage scenario. These processes are assigned to the cgroup, which ensures that they
 * are subject to the memory limit. The program handles SIGINT and SIGTERM signals to gracefully terminate the
 * stress-ng processes and release resources before exiting. The purpose of this program is to demonstrate memory
 * management in Linux using cgroups, simulate memory pressure using `stress-ng`, and observe the effects of
 * memory constraints under controlled conditions.
 */
